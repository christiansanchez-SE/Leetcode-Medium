...
var intToRoman = function(num) {
    // Define mappings from integers to their Roman numeral symbols
    const valueSymbols = [
        { value: 1000, symbol: "M" },
        { value: 900, symbol: "CM" },
        { value: 500, symbol: "D" },
    ...
    let romanNumeral = "";  // Result string for Roman numeral

    // Loop through the value-symbol pairs
    for (let { value, symbol } of valueSymbols) {
        // Append the symbol to the result for each complete value in num
        while (num >= value) {
            romanNumeral += symbol;
            num -= value;
        }
...
var divide = function(dividend, divisor) {
    ...
    if (dividend === MIN_INT && divisor === 1) {
        return MIN_INT; // Handle this specific edge case
    }

    // Determine the sign of the result
    const isNegative = (dividend < 0) !== (divisor < 0);

    // Use unsigned values for calculation to avoid issues with Math.abs()
    let absDividend = dividend < 0 ? -dividend : dividend;
    let absDivisor = divisor < 0 ? -divisor : divisor;

    let quotient = 0;

    // Directly calculate quotient using bit manipulation
    for (let i = 31; i >= 0; i--) {
    ...
    // Apply sign and ensure the result is within 32-bit range
    quotient = isNegative ? -quotient : quotient;
    return Math.max(MIN_INT, Math.min(MAX_INT, quotient));
};

console.log(divide(-2147483648, 1)); // Output: -2147483648
console.log(divide(-2147483648, -1)); // Output: 2147483647
console.log(divide(2147483647, 2)); // Output: 1073741823
